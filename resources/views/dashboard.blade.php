...
<body class="bg-slate-50 font-sans text-slate-800">
    ...
    <main class="pt-32 pb-20 container mx-auto px-4 flex justify-center">

        <aside class="w-full lg:w-1/3">
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 text-center flex flex-col items-center justify-center">
                ...
                <div class="flex justify-center items-center w-full mb-4">
                    <div class="relative w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-lg ring-2 ring-indigo-100">
                        @if(isset($user_foto) && $user_foto && file_exists(public_path('user_foto/' . $user_foto)))
                            <img src="{{ asset('user_foto/' . $user_foto) }}" alt="Foto Profil" class="w-full h-full object-cover">
                        @else
                ...
                <div class="border-t border-slate-100 pt-6 space-y-4 w-full">
                    <a href="{{ url('/tes') }}" class="block w-full py-3.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition duration-200 text-center text-sm shadow-sm">Ambil Tes Baru</a>
                    <a href="{{ url('/kampus') }}" class="block w-full py-3.5 bg-blue-100 border border-blue-200 text-slate-700 font-bold rounded-xl text-center text-sm transition-all duration-300 ease-in-out hover:bg-blue-200 hover:text-slate-900">Eksplorasi Kampus</a>
                    <form action="{{ route('logout') }}" method="POST" class="w-full">
                        @csrf
                        <button type="submit" class="w-full py-3.5 bg-red-50 border border-red-100 text-red-600 font-bold rounded-xl text-center text-sm transition-all duration-300 ease-in-out hover:bg-red-100">Logout Akun</button>
                    </form>
                </div>
            </div>
        </aside>
