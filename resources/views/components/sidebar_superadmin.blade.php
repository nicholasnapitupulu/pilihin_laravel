<div class="w-64 bg-slate-900 text-white min-h-screen flex flex-col justify-between p-6 shadow-xl">
    <div>
        ...
        <nav class="space-y-2">
            ...
            {{-- Kelola Data Admin --}}
            @if(request()->routeIs('superadmin.admin.*'))
            ...
            @if(request()->routeIs('superadmin.users.*'))
                <a href="{{ route('superadmin.users.index') }}" class="flex items-center gap-3 px-4 py-3 bg-purple-600 rounded-xl font-bold text-sm text-white shadow-md transition duration-200">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
